...
public class EqualSolutions implements Comparator {
    ...
    public int compare(Object object1, Object object2) {
        ...
        if (p1Objetivo1 < p2Objetivo1) {
            flag = -1;
        } else if (p1Objetivo1 > p2Objetivo1) {
            flag = 1;
        } else {
            flag = 0;
        }
        if (flag == -1) {
            dominate2++;//2 domina en este objetivo
        }
        if (flag == 1) {
            dominate1++;//1 domina en este objetivo
        }
        p1Objetivo2 = solution1.getObjective(1);
        p2Objetivo2 = solution2.getObjective(1);
        if (p1Objetivo2 < p2Objetivo2) {
            flag = -1;
        } else if (p1Objetivo2 > p2Objetivo2) {
            flag = 1;
        } else {
            flag = 0;
        }
        if (flag == -1) {
            dominate2++;//1 domina en este objetivo
        }
        if (flag == 1) {
            dominate1++;//2 domina en este objetivo
        }
        if (dominate1 == 0 && dominate2 == 0) {
            return 0;//No se dominan (iguales, es lo que importa acá)
        }
        return 2;
    }
