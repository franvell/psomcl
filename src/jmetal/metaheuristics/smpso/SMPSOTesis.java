...
public class SMPSOTesis extends Algorithm {
    ...
    public void iniciarParametros() {
        tamanhoEnjambre = ((Integer) getInputParameter("tamanhoEnjambre")).intValue();
        tamanhoLideres = ((Integer) getInputParameter("tamanhoLideres")).intValue();
        maximoIteraciones = ((Integer) getInputParameter("maximoIteraciones")).intValue();
        mutacion = operators_.get("mutation");
        iteracion = 0;
        exito = false;
        particulas = new SolutionSet(tamanhoEnjambre);//coleccion de particulas
        mejoresLocales = new Solution[tamanhoEnjambre];//vector de mejores locales
        mejoresGlobales = new CrowdingArchive(tamanhoLideres, problem_.getNumberOfObjectives());//lideres global best
    ...
    private double restringirAceleracion(double c1, double c2) {
        double rho = c1 + c2;
        if (rho <= 4) {
            return 1.0;//si la suma de las constantes es menor o igual a 4 retorna 1
        } else {
            return 2 / (2 - rho - Math.sqrt(Math.pow(rho, 2.0) - 4.0 * rho));//sino, 2/(2-(c1+c2)-raiz cuadrada de [(c1+c2) al cuadrado - 4*(c1+c2)])
        }
    }
    ...
    private double restringirVelocidad(double velocidad, double[] velMaxima, double[] velMinima, int variableIndex, int particleIndex) throws IOException {
        ...
        resultado = velocidad;
        if (velocidad > maximo) {
            resultado = maximo;//este es el limite maximo de velocidad, si se supera, se retorna el delta max
        }
        if (velocidad < minimo) {
            resultado = minimo;//igual q max pero para limite minimo..
        }
        return resultado;
    }
    ...
    private void calcularVelocidad(int numeroIteracion, int maximoIteraciones) {
        try {
            double r1, r2, C1, C2;
            double wmax, wmin;
            XReal mejorGlobal;
            for (int i = 0; i < tamanhoEnjambre; i++) {
                //por cada particula hacemos el calculo de velocidad
            ...
            log.error("Error al calcular las velocidades..", ex);
    ...
    private void calcularNuevasPosiciones() throws JMException {
        for (int i = 0; i < tamanhoEnjambre; i++) {//por cada particula del swarm
    ...
    private void mutar(int actualIteration, int totalIterations) throws JMException {
        for (int i = 0; i < particulas.size(); i++) {//por cada particula se puede hacer una mutacion o turbulencia
            if ((i % 6) == 0) {
                mutacion.execute(particulas.get(i));// si se cumple, se muta
            }
        }
    }
    ...
    public SolutionSet execute() throws ClassNotFoundException, JMException {
        ...
        log.info("-------------------------------------------------------");
        log.info("STEP 4 and 5. lideres iniciales. archivo de global best inicial, solo entraran los dominantes");
        ...
        final Syncronizador sync = new Syncronizador();
        while (iteracion < maximoIteraciones) {
            iteracion++;
            final Thread thread = new Thread("iteracion:" + iteracion) {
                public void run() {
                    try {
                        int hilos;
                        synchronized (sync) {
                        ...
                        System.out.println("run hilo >> " + hilos);

                        log.info("run hilo >> " + hilos + " Iteracion [" + (iteracion) + " de " + maximoIteraciones + "] " + Calendar.getInstance().getTime().toString());

                        calcularVelocidad(iteracion, maximoIteraciones);
                        log.info("run hilo >> " + hilos + " Iteracion [" + (iteracion) + " de " + maximoIteraciones + "] Calculo las velocidades");

                        calcularNuevasPosiciones();
                        log.info("run hilo >> " + hilos + " Iteracion [" + (iteracion) + " de " + maximoIteraciones + "] Calculo las nuevas posiciones");
                        ...
                        for (int i = 0; i < particulas.size(); i++) {//por cada particula
                            ...
                            log.info("run hilo >> " + hilos + " Particula [" + (i + 1) + " de " + particulas.size() + "] OBJ. contraste:" + particula.getObjective(0) + ", entropia:" + particula.getObjective(1) + " VARDESC. n:" + particula.getDecisionVariables()[1].toString() + ", m:" + Configuration.mMax + ", alfa:" + particula.getDecisionVariables()[3].toString() + ", type:" + particula.getDecisionVariables()[2].toString() + ", dim:" + particula.getDecisionVariables()[0].toString());
                        }
                        ...
                        distance_.crowdingDistanceAssignment(mejoresGlobales, problem_.getNumberOfObjectives());
                        System.out.println("run hilo END >> " + hilos);
                    } catch (Exception e) {
                        System.out.print(e);
                        synchronized (sync) {
                            sync.sub();
                            System.out.println("hilo " + sync.valor().intValue());
                        }
                    }
                }
            };
